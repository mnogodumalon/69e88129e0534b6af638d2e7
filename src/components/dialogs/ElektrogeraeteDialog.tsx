...
interface ElektrogeraeteDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Elektrogeraete['fields']) => Promise<void>;
  defaultValues?: Elektrogeraete['fields'];
...
export function ElektrogeraeteDialog({ open, onClose, onSubmit, defaultValues, kategorienList, standorteList, enablePhotoScan = true, enablePhotoLocation = true }: ElektrogeraeteDialogProps) {
  const [fields, setFields] = useState<Partial<Elektrogeraete['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'elektrogeraete');
      await onSubmit(clean as Elektrogeraete['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["kategorie", "standort"]);
        ...
        return merged as Partial<Elektrogeraete['fields']>;
      });
  ...
  const DIALOG_INTENT = defaultValues ? 'Elektrogeräte bearbeiten' : 'Elektrogeräte hinzufügen';
    ...
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="garantie_ablauf">Garantieablauf</Label>
            <Input
              id="garantie_ablauf"
              type="date"
              value={fields.garantie_ablauf ?? ''}
              onChange={e => setFields(f => ({ ...f, garantie_ablauf: e.target.value }))}
            />
          </div>
          <div className="space-y-2">
            ...
            >
              ...
              <SelectContent>
                ...
                <SelectItem value="lagernd">Lagernd</SelectItem>
              </SelectContent>
            </Select>
          </div>
          <div className="space-y-2">
            <Label htmlFor="foto">Foto des Geräts</Label>
            {fields.foto ? (
