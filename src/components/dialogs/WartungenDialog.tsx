...
interface WartungenDialogProps {
  ...
  defaultValues?: Wartungen['fields'];
  elektrogeraeteList: Elektrogeraete[];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function WartungenDialog({ open, onClose, onSubmit, defaultValues, elektrogeraeteList, enablePhotoScan = true, enablePhotoLocation = true }: WartungenDialogProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'wartungen');
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      contextParts.push(`<available-records field="geraet" entity="Elektrogeräte">\n${JSON.stringify(elektrogeraeteList.map(r => ({ record_id: r.record_id, ...r.fields })), null, 2)}\n</available-records>`);
      if (usePersonalInfo) {
        try {
      ...
      const schema = `{\n  "geraet": string | null, // Display name from Elektrogeräte (see <available-records>)\n  "wartungsart": LookupValue | null, // Wartungsart (select one key: "inspektion" | "reparatur" | "kalibrierung" | "reinigung" | "software_update" | "sonstiges") mapping: inspektion=Inspektion, reparatur=Reparatur, kalibrierung=Kalibrierung, reinigung=Reinigung, software_update=Software-Update, sonstiges=Sonstiges\n  "wartungsdatum": string | null, // YYYY-MM-DD\n  "techniker_vorname": string | null, // Techniker Vorname\n  "techniker_nachname": string | null, // Techniker Nachname\n  "beschreibung": string | null, // Durchgeführte Arbeiten\n  "ergebnis": LookupValue | null, // Ergebnis (select one key: "erfolgreich" | "teilweise_erfolgreich" | "nicht_erfolgreich" | "handlungsbedarf") mapping: erfolgreich=Erfolgreich, teilweise_erfolgreich=Teilweise erfolgreich, nicht_erfolgreich=Nicht erfolgreich, handlungsbedarf=Weiterer Handlungsbedarf\n  "naechste_wartung": string | null, // YYYY-MM-DD\n  "kosten": number | null, // Kosten (€)\n  "wartung_notizen": string | null, // Notizen\n}`;
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["geraet"]);
        for (const [k, v] of Object.entries(raw)) {
          if (applookupKeys.has(k)) continue;
          if (v != null) merged[k] = v;
        }
        const geraetName = raw['geraet'] as string | null;
        if (geraetName) {
          const geraetMatch = elektrogeraeteList.find(r => matchName(geraetName!, [String(r.fields.geraet_name ?? '')]));
          if (geraetMatch) merged['geraet'] = createRecordUrl(APP_IDS.ELEKTROGERAETE, geraetMatch.record_id);
        }
  ...
  const DIALOG_INTENT = defaultValues ? 'Wartungen bearbeiten' : 'Wartungen hinzufügen';
      ...
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="geraet">Gerät</Label>
            <Select
              value={extractRecordId(fields.geraet) ?? 'none'}
              onValueChange={v => setFields(f => ({ ...f, geraet: v === 'none' ? undefined : createRecordUrl(APP_IDS.ELEKTROGERAETE, v) }))}
            >
              <SelectTrigger id="geraet"><SelectValue placeholder="Auswählen..." /></SelectTrigger>
              <SelectContent>
                <SelectItem value="none">—</SelectItem>
                {elektrogeraeteList.map(r => (
