...
export default function ElektrogeraetePage() {
  const [records, setRecords] = useState<Elektrogeraete[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Elektrogeraete | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Elektrogeraete | null>(null);
  const [viewingRecord, setViewingRecord] = useState<Elektrogeraete | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      const [mainData, kategorienData, standorteData] = await Promise.all([
  ...
  async function handleCreate(fields: Elektrogeraete['fields']) {
    await LivingAppsService.createElektrogeraeteEntry(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: Elektrogeraete['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updateElektrogeraeteEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteElektrogeraeteEntry(deleteTarget.record_id);
  ...
  return (
    <PageShell
      title="Elektrogeräte"
      subtitle={`${records.length} Elektrogeräte im System`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder="Elektrogeräte suchen..."
          value={search}
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('garantie_ablauf')}>
                <span className="inline-flex items-center gap-1">
                  Garantieablauf
                  {sortKey === 'garantie_ablauf' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; setViewingRecord(record); }}>
                ...
                <TableCell className="text-muted-foreground">{formatDate(record.fields.garantie_ablauf)}</TableCell>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={14} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Elektrogeräte. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
