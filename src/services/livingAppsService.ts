...
import type { Kategorien, Standorte, Elektrogeraete, Wartungen, CreateKategorien, CreateStandorte, CreateElektrogeraete, CreateWartungen } from '@/types/app';
...
export class LivingAppsService {
  ...
  // --- ELEKTROGERAETE ---
  static async getElektrogeraete(): Promise<Elektrogeraete[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.ELEKTROGERAETE}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Elektrogeraete[];
    return enrichLookupFields(records, 'elektrogeraete');
  }
  static async getElektrogeraeteEntry(id: string): Promise<Elektrogeraete | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.ELEKTROGERAETE}/records/${id}`);
    const record = { record_id: data.id, ...data } as Elektrogeraete;
    return enrichLookupFields([record], 'elektrogeraete')[0];
  }
  static async createElektrogeraeteEntry(fields: CreateElektrogeraete) {
    return callApi('POST', `/apps/${APP_IDS.ELEKTROGERAETE}/records`, { fields: cleanFieldsForApi(fields as any, 'elektrogeraete') });
  }
  static async updateElektrogeraeteEntry(id: string, fields: Partial<CreateElektrogeraete>) {
    return callApi('PATCH', `/apps/${APP_IDS.ELEKTROGERAETE}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'elektrogeraete') });
  }
  static async deleteElektrogeraeteEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.ELEKTROGERAETE}/records/${id}`);
  }
  ...
  static async getWartungen(): Promise<Wartungen[]> {
    ...
    return enrichLookupFields(records, 'wartungen');
  }
  static async getWartungenEntry(id: string): Promise<Wartungen | undefined> {
    ...
    const record = { record_id: data.id, ...data } as Wartungen;
    return enrichLookupFields([record], 'wartungen')[0];
  }
  static async createWartungenEntry(fields: CreateWartungen) {
    return callApi('POST', `/apps/${APP_IDS.WARTUNGEN}/records`, { fields: cleanFieldsForApi(fields as any, 'wartungen') });
  }
  static async updateWartungenEntry(id: string, fields: Partial<CreateWartungen>) {
    return callApi('PATCH', `/apps/${APP_IDS.WARTUNGEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'wartungen') });
  }
